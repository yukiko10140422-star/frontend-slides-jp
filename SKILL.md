---
name: frontend-slides-jp
description: Japanese-enhanced fork of frontend-slides. Create stunning, animation-rich HTML presentations with full Japanese typography support, 6 Japanese-optimized style presets, and a single-source pipeline that generates HTML + text-selectable PDF + editable PPTX simultaneously. Use when the user wants to build a presentation, convert a PPT/PPTX to web, or create Japanese slides for a talk/pitch/research report.
---
...
- **Mode D: Markdown/Doc Source** — Generate slides from an existing markdown file, research report, blog post, or text document the user points to. Go to Phase 1 Auto.

### Auto Mode Trigger (質問ゼロモード)

If the user explicitly says any of:
- 「質問ゼロで」「質問なしで」「勝手に決めて」「お任せで」
- "no questions", "just make it", "auto"
- Provides a specific file and says "this" / "これを" without negotiating style

Then enter **Auto Mode**: skip all AskUserQuestion calls, infer every decision from the content, and generate directly. Announce the inferred decisions in a single short status line before generating (e.g., "日本語・JP-2 Dark Tech・15 スライド・編集無しで生成します"). See Phase 1 Auto below.

## Phase 0.5: Language Detection (言語検出)

**Before Phase 0.7**, determine presentation language. This changes font loading, viewport rules, and style preset selection.

**Japanese mode triggers:**
- User's message contains Japanese characters (ひらがな・カタカナ・漢字)
- User explicitly says "日本語で" / "Japanese"
- Provided content (markdown / PPTX / text file) contains Japanese text
- Target audience is clearly Japanese (国内 B2B, IR, 官公庁, 日本市場)

**When Japanese mode is active, you MUST:**
1. Read [JAPANESE.md](JAPANESE.md) before generating anything
2. Use `<html lang="ja">` in the output
3. Load Japanese Google Fonts (`Noto Sans JP` at minimum)
4. Prefer Japanese presets from [STYLE_PRESETS.md](STYLE_PRESETS.md) (JP-1 through JP-6)
5. Apply the stricter Japanese content density limits from JAPANESE.md §5
6. Pass the Japanese checklist from JAPANESE.md §8 before delivering

If unclear, default to Japanese when the user's messages are in Japanese. Don't ask — infer.

## Phase 0.7: Output Format Selection (出力形式の選択) ⚠️ MANDATORY FIRST QUESTION

**This is the FIRST thing to ask** (after detecting mode + language, before content discovery). Output format changes the entire generation pipeline, so it must be decided upfront.

**Ask via AskUserQuestion (header: "Format"):**

> どの形式で成果物を作成しますか？

Options:

- **All (recommended)** — HTML + PDF + PPTX all three. Uses multi-format pipeline. Best for deliverables.
- **HTML only** — Single self-contained HTML file with animations. Browser presentation. Fastest.
- **HTML + PDF** — HTML for presenting, PDF for sharing/printing. Text-selectable PDF.
- **HTML + PPTX** — HTML for presenting, PPTX for PowerPoint editing. Native editable text/shapes.
- **PDF only** — Just the text-selectable PDF (generated via HTML intermediate).
- **PPTX only** — Just the editable PPTX for PowerPoint.

### Pipeline selection based on format choice

| User's choice | Pipeline | Files generated |
|---|---|---|
| **HTML only** | Single-file inline (legacy Phase 3 path) | `presentation.html` |
| **HTML + PDF** or **PDF only** | Multi-format scaffold | `index.html` + `<name>.pdf` |
| **HTML + PPTX** or **PPTX only** | Multi-format scaffold | `index.html` + `<name>.pptx` |
| **All** | Multi-format scaffold | `index.html` + `<name>.pdf` + `<name>.pptx` |

### Auto Mode default

If user is in **Auto Mode** (質問ゼロ), default to **All (HTML + PDF + PPTX)** because it's the most useful deliverable set and the user's durable preference is to archive everything to Obsidian Vault.

Announce the decision in a single line before proceeding:
> "日本語・JP-2 Dark Tech・12 スライド・HTML+PDF+PPTX 全形式・Vault 03-Research/ に配置で生成します。"

## Phase 0.8: Multi-Format Scaffold Setup

**If the user selected anything other than "HTML only", follow this procedure:**

1. **Copy the scaffold** from `scripts/multi-format/` to the output directory (typically a subfolder under Obsidian Vault `03-Research/` or `02-Projects/`):

   ```bash
   SCAFFOLD=~/.claude/skills/frontend-slides/scripts/multi-format
   OUT="$VAULT/03-Research/<topic>"
   mkdir -p "$OUT"
   cp "$SCAFFOLD"/{render-html.mjs,build-pdf.mjs,build-pptx.mjs,package.json} "$OUT/"
   ```

2. **Write `slides-data.mjs`** in the output directory based on content from Phase 1 and style from Phase 2. Use `slides-data.example.mjs` as a reference for slide types.

3. **Install dependencies** (first run only):
   ```bash
   cd "$OUT" && npm install
   ```
   This installs Playwright (+ Chromium shell) and PptxGenJS.

4. **Run the build** based on what the user selected:
   ```bash
   npm run build         # all three
   npm run build:html    # HTML only
   npm run build:pdf     # PDF only (depends on HTML existing)
   npm run build:pptx    # PPTX only
   ```

5. **Verify outputs** and open them for the user.

6. **Do NOT copy `node_modules`** to Obsidian Vault subsequently — it bloats iCloud sync. Keep it in the same folder so rebuilds work, but remember to `.gitignore` / exclude it from any syncs.

### Slide type vocabulary (must match across data + renderers)
...
| `two-col-bullets` | Comparison + supporting bullets |
| `kpi` | 3 metric cards |
| `layers` | Vertical architecture stack |
| `flow` | Horizontal step flow |
| `lead-bullets` | Lead paragraph + bullets |
| `bullets` | Pure bullet list |
| `phases` | 3-phase roadmap |
| `closing` | Final thesis / recommendation |

See `scripts/multi-format/slides-data.example.mjs` for the exact shape of each type.

### Feedback rule: always deliver to Obsidian Vault

Completed deliverables MUST be placed inside the user's Obsidian Vault subfolder (research → `03-Research/<topic>/`, project → `02-Projects/<name>/`). Never leave deliverables in `Downloads/` or `system32`. Also create an index note (`<topic>.md`) with frontmatter, wikilinks, and `![[...]]` embeds for the PDF.
...
| -------------------------------------------------- | -------------------------------------------------------------------- | ------------------------- |
| [JAPANESE.md](JAPANESE.md)                         | 日本語スライド専用ガイド（フォント・禁則・密度・検証）                 | **Phase 0.5 (日本語なら必読)** |
| [STYLE_PRESETS.md](STYLE_PRESETS.md)               | 12 curated visual presets + 6 日本語最適化プリセット (JP-1〜JP-6)    | Phase 2 (style selection) |
| [viewport-base.css](viewport-base.css)             | Mandatory responsive CSS (日本語オーバーライド含む) — copy into every presentation | Phase 3 (generation) |
| [html-template.md](html-template.md)               | HTML structure, JS features, code quality standards                  | Phase 3 (generation)      |
| [animation-patterns.md](animation-patterns.md)     | CSS/JS animation snippets and effect-to-feeling guide                | Phase 3 (generation)      |
| [scripts/extract-pptx.py](scripts/extract-pptx.py) | Python script for PPT content extraction                             | Phase 4 (conversion)      |
| [scripts/deploy.sh](scripts/deploy.sh)             | Deploy slides to Vercel for instant sharing                          | Phase 6 (sharing)         |
| [scripts/export-pdf.sh](scripts/export-pdf.sh)     | Export slides to PDF (screenshot-based, image PDF)                   | Phase 6 (legacy)          |
| [scripts/multi-format/](scripts/multi-format/)     | **Single-source → HTML + text-selectable PDF + editable PPTX pipeline** | **Phase 0.8 (when PDF/PPTX needed)** |
| [scripts/multi-format/slides-data.example.mjs](scripts/multi-format/slides-data.example.mjs) | Example data file with all 11 slide types             | Reference when writing content |
| [scripts/multi-format/render-html.mjs](scripts/multi-format/render-html.mjs) | HTML renderer (reads slides-data.mjs)                    | Phase 0.8 (generation)    |
| [scripts/multi-format/build-pdf.mjs](scripts/multi-format/build-pdf.mjs) | Playwright `page.pdf()` — text-selectable vector PDF        | Phase 0.8 (PDF output)    |
| [scripts/multi-format/build-pptx.mjs](scripts/multi-format/build-pptx.mjs) | PptxGenJS native text + shapes — fully editable PPTX      | Phase 0.8 (PPTX output)   |
| [scripts/multi-format/README.md](scripts/multi-format/README.md) | How to use the scaffold                                             | Phase 0.8 (setup)         |
